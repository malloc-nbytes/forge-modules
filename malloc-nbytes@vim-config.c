...
char *getdesc(void) { return "My config for vim"; }
...
int install(void) {
        ...
        forge_str_concat(&config_fp, "/.vimrc");
        ...
        char *buf = forge_cstr_builder("cp ./.vimrc ", config_fp.data, NULL);
        cmd(buf);

        change_file_owner(config_fp.data, user);
